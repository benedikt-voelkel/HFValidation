...
//run numbers
const Int_t nruns = 1;
Int_t runlist[nruns] = {289242};

//Task configuration
TString cutFile="../tree.root";          // file containing the cuts for the different meso
...
void run(Bool_t isRunOnMC=kFALSE)
{
  //TGrid::Connect("alien://");
  ...
  if (isRunOnMC == kFALSE){
    pathToLocalAODfiles = "../";
    gridDataDir="/alice/data/2018/LHC18l";
    System=kpp;
  }
...
//  }
  ...
  AliAnalysisTaskSEHFTreeCreator *task = reinterpret_cast<AliAnalysisTaskSEHFTreeCreator*>(gInterpreter->ProcessLine(Form(".x %s (%d,%d,\"%s\",\"%s\", %d,%d,%d,%d,%d,%d,%d,%d,%d, %d)",gSystem->ExpandPathName("$ALICE_PHYSICS/PWGHF/treeHF/macros/AddTaskHFTreeCreator.C"),isRunOnMC,0,"HFTreeCreator",cutFile.Data(),1,kFALSE,kFALSE,0,0,0,0,1,0,0)));
  //cri task 
  AliAnalysisTask *Lc=reinterpret_cast<AliAnalysisTask*>(gInterpreter->ProcessLine(Form(".x %s",gSystem->ExpandPathName(Form("AddTaskLambdac.C(\"%s\",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\"%s\")","../Lctask.root",kTRUE,kFALSE,kFALSE,kTRUE,kFALSE,kFALSE,kTRUE,kFALSE,kFALSE,0,1,"test")))));
...
//  }
  ...
  AliAnalysisTaskSEHFTreeCreator *task = AddTaskHFTreeCreator(isRunOnMC, 0, "HFTreeCreator", cutFile.Data(), 1, kFALSE, kFALSE);
  ...
  if(local) {
    ...
    for(Int_t i=1;i<2;i++){
    chainAOD->Add(Form("%s/00%d/AliAOD.root",pathToLocalAODfiles.Data(), i));
    chainAODfriend->Add(Form("%s/00%d/AliAOD.VertexingHF.root",pathToLocalAODfiles.Data(), i));
    }
    chainAOD->AddFriend(chainAODfriend);
    
    
    // start the analysis locally, reading the events from the tchain
    mgr->StartAnalysis("local", chainAOD, 1000000, 0);
    mgr->SetNSysInfo(5);
    mgr->PrintStatus();
    mgr->SetUseProgressBar(1, 5);
    if(mgr->GetNcalls()==50) mgr->Terminate();
